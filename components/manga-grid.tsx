...
export default function MangaGrid({ manga, currentPage, totalPages, searchParams }: MangaGridProps) {
  ...
  const createPageUrl = (page: number) => {
    ...
    return `/manga/browse?${params.toString()}`
  }
    ...
    <div className="space-y-6">
      {manga.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <Button asChild>
            <Link href="/manga/browse">Clear Filters</Link>
          </Button>
        </div>
      )}
    </div>
